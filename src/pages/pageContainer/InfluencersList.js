...
const InfluencersList = (props) => {
  ...
  console.log("showPopshowPop", showPop)
        ...
        DropDown: (
          ...
          >
            <Dropdown.Item
              eventKey="1"
              onClick={async () => {
                dispatch(await influencerUpdateStatus(2, item?.email, true));
            ...
            <Dropdown.Item
              eventKey="req"
              onClick={async () => {
                dispatch(await influencerUpdateStatus(3, item?.email, false));
                dispatch(getInfluencer());
              }}
            >
              Inactive
            </Dropdown.Item>
            <EditInfluencers name={item?.name} email={item?.email} promo={item?.promo} code={item?.code} source={item?.source}/>
            <Dropdown.Item
              eventKey="req"
              onClick={async () => {
                dispatch(await deleteInfluencer(item?.email));
                dispatch(getInfluencer());
              }}
            >
              Delete
